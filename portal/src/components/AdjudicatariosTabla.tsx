...
export default function AdjudicatariosTabla() {
  ...
  const cargar = useCallback(() => {
    setLoading(true);
    api.adjudicatarios(q || undefined, pagina, POR_PAGINA)
  ...
  return (
    <div className="adjudicatarios-tabla space-y-4">
      ...
      {datos && (
        <p className="tabla-total text-xs text-muted-foreground">
          {datos.total.toLocaleString('es-ES')} empresas
          {q && ` · filtrando por «${q}»`}
        </p>
      )}
      ...
      {loading ? <Skeleton /> : (
        <div className="tabla-contenedor rounded-md border overflow-x-auto">
          <table className="adjudicatarios-tabla-datos w-full text-sm">
            <thead>
            ...
            <tbody>
              {resultados.map((a, i) => (
                <tr key={a.adjudicatario_id + i} className={i % 2 === 1 ? 'adjudicatario-row bg-muted/20' : 'adjudicatario-row'}>
                  <td className="px-4 py-3">
                    <a
                      href={`/adjudicatarios/${encodeURIComponent(a.adjudicatario_id)}`}
                      className="font-medium hover:underline"
                    >
                      {a.adjudicatario_nombre}
                    </a>
                    {a.adjudicatario_nif && (
                      <span className="block text-xs text-muted-foreground/60 font-mono">{a.adjudicatario_nif}</span>
                    )}
                  </td>
                  <td className="px-4 py-3 text-right tabular-nums font-mono text-muted-foreground">
                    {a.num_contratos.toLocaleString('es-ES')}
                  </td>
                  <td className="px-4 py-3 text-right tabular-nums font-mono">
                    {formatEuro(a.total_adjudicado)}
                  </td>
                  <td className="px-4 py-3 text-xs text-muted-foreground">
                    {yearDesde(a.primer_contrato)}
                  </td>
                </tr>
              ))}
            </tbody>
